...
ARG CC=${CROSS_COMPILE}-gcc
ARG CXX=${CROSS_COMPILE}-g++

# Base image
# ----------
FROM ghcr.io/arduino/crossbuild-base:${BASE_VERSION} AS base

# Download cross compiler if required
ARG DEB_PACKAGES
RUN if [ -n "$DEB_PACKAGES" ]; then apt-get install -y $DEB_PACKAGES; fi
...
RUN cmake -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DCMAKE_C_COMPILER="$CC" -DCMAKE_CXX_COMPILER="$CXX" -DCMAKE_INSTALL_PREFIX="${PREFIX}" -DSHAREDLIBS=OFF -DBUILD_TESTS=OFF -DPYTHON_BINDINGS=OFF -DEXAMPLES=OFF -DFTDI_EEPROM=OFF ..
RUN make
RUN make install
...
# Build eudev-3.2.14
WORKDIR /build
RUN wget https://github.com/eudev-project/eudev/releases/download/v3.2.14/eudev-3.2.14.tar.gz
RUN tar xfv eudev-3.2.14.tar.gz
WORKDIR /build/eudev-3.2.14
RUN ./configure --enable-static --disable-gudev --disable-introspection --disable-shared --disable-blkid --disable-kmod --disable-manpages --prefix="$PREFIX" --host="${CROSS_COMPILE}"
RUN make -j$(nproc)
RUN make install
